...
const pricingPlans = [
  {
    ...
    description: "A focused operational module for one business domain and one implementation environment.",
    price: "$69",
    duration: "Month",
    ...
    features: [
      ...
      { name: "6 months support.", included: true, highlight: true },
      { name: "Permitted for one domain.", included: true },
      { name: "For one business project.", included: true },
      { name: "Support within 2 business days.", included: true },
  ...
  },
  {
    ...
    description: "A full commercial license for teams that need source access, installation help, and faster support.",
    price: "$999",
    duration: "Month",
    ...
    features: [
      ...
      { name: "Support within 1 business day.", included: true },
...
export function PricingSection() {
  return (
    <section id="pricing" className="pricing-section relative mx-auto max-w-7xl px-6 py-24 lg:px-8 bg-white z-20" data-reveal>
      <div className="mx-auto mb-16 max-w-3xl text-center">
        <p className="section-kicker">Pricing</p>
        <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-black leading-tight">
          Start focused, then expand with the source
        </h2>
        <p className="mx-auto mt-6 text-lg text-gray-600 leading-relaxed">
          Pick the license that matches your rollout plan. Custom implementation support is available when your workflow needs deeper configuration.
        </p>
      </div>
            ...
            {pricingPlans[0].features.map((feature, idx) => (
              <li key={idx} className="flex items-center gap-3 text-sm">
                ...
                <span className={feature.included ? "text-gray-600" : "text-gray-400"}>
                  {feature.highlight ? (
                    <>6 months <span className="text-[#8c181b]">support.</span></>
                  ) : feature.name}
                </span>
              </li>
            ))}
            ...
            {pricingPlans[1].features.map((feature, idx) => (
              <li key={idx} className="flex items-center gap-3 text-sm">
                ...
                <span className="text-gray-100">
                  {feature.highlight ? (
                    <>6 months <span className="text-red-400">support.</span></>
                  ) : feature.name}
                </span>
              </li>
            ))}
